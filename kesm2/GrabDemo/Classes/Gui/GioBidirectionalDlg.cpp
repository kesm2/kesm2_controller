...
CString strEventCount;
    ...
    strEventCount.Format(_T("%d"), pInfo->GetEventCount());
...
BOOL CGioBidirectionalDlg::OnInitDialog() 
{
   ...
   if( !strstr( label, "bidirectional") && !strstr( label, "Bidirectional"))
   {
   ...
   m_pGio->GetParameter(CORGIO_PRM_DIR_OUTPUT, &prmDirection);
	for (iIO=0; iIO < (UINT32)m_pGio->GetNumPins(); iIO++)
	{
      if(!(capReadOnly & (1 << iIO)))
      {
   		((CComboBox*)GetDlgItem(IDC_SCG_DIR1 + iIO))->AddString(_T("Input"));
         ((CComboBox*)GetDlgItem(IDC_SCG_DIR1 + iIO))->AddString(_T("Output"));
      ...
      {
   		((CComboBox*)GetDlgItem(IDC_SCG_DIR1 + iIO))->AddString(_T("Reserved"));
         ((CComboBox*)GetDlgItem(IDC_SCG_DIR1 + iIO))->SetCurSel(0);
      }
	}
	...
	if (capEventType & CORGIO_VAL_EVENT_TYPE_RISING_EDGE)
		m_Combo_Signal_Event.AddString(_T(RISING_EDGE_CALLBACK));
	if (capEventType & CORGIO_VAL_EVENT_TYPE_FALLING_EDGE)
		m_Combo_Signal_Event.AddString(_T(FALLING_EDGE_CALLBACK));
	if (capEventType & CORGIO_VAL_EVENT_TYPE_FAULT)
		m_Combo_Signal_Event.AddString(_T(FAULT_CALLBACK));
	if (capEventType)
...
void CGioBidirectionalDlg::OnEnableCallback() 
{
	...
	iSelection = m_Combo_Signal_Event.GetCurSel();
	m_Combo_Signal_Event.GetLBText(iSelection,CString(str));
