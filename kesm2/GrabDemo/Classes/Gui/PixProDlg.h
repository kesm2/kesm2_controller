...
protected:
	...
	void SaveSettings();
	void SetDirectories();
	void UpdateCurrentDir(TCHAR *newCurrentDir);
	void UpdateNames();
...
protected:
	...
	TCHAR m_currentDir[MAX_PATH];			// Directory where to find the CPI files
	TCHAR m_currentFileName[MAX_PATH];	// File name (without path) of the current file selected
	int m_currentFileIndex;					// Index of the file name in the list current selected

	TCHAR m_oldCurrentDir[MAX_PATH];			// Old Directory where to find the CPI files
	TCHAR m_oldCurrentFileName[MAX_PATH];	// Old File name (without path) of the current file selected
