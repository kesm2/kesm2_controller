...
BOOL CFlatFieldDlg::OnInitDialog() 
{
	...
	{
		AfxMessageBox(_T("Invalid buffer object"));
		CDialog::OnCancel();
		return FALSE;
	}
	...
	{
		AfxMessageBox(_T("Invalid flat-field object"));
		CDialog::OnCancel();
		return FALSE;
	}
   ...
   str.Format(_T("Step 1: %s a dark image"), isOffLine ? _T("Load"): _T("Acquire"));
   GetDlgItem(IDC_SCG_FF_DARK_IMAGE_MSG_1)->SetWindowText(str);

   str.Format(_T("%s &Dark Image"), isOffLine ? _T("Load"): _T("Acquire"));
   GetDlgItem(IDC_SCG_FF_DARK_IMAGE)->SetWindowText(str);

   str.Format(_T("Step 2: %s a non-saturated bright image"), isOffLine ? _T("Load"): _T("Acquire"));
   GetDlgItem(IDC_SCG_FF_BRIGHT_IMAGE_MSG_1)->SetWindowText(str);

   str.Format(_T("%s &Bright Image"), isOffLine ? _T("Load"): _T("Acquire"));
   ...
   str.Format(_T("(recommended average pixel\nvalue below %d)\n"), m_RecommendedDark);
   ...
   // Set correction type
   m_CorrectionTypeCtrl.AddString(_T("Flat Field"));
   m_CorrectionTypeCtrl.AddString(_T("Flat Line"));
   m_CorrectionTypeCtrl.SetCurSel(m_CorrectionType == SapFlatField::CorrectionTypeField ? 0 : 1);

	// Set video type
	m_VideoTypeCtrl.AddString( _T("Monochrome"));
	m_VideoTypeCtrl.AddString( _T("Bayer"));
   ...
   if (m_pFlatField->GetAcqDevice())
   {
      ...
      if(status)
      {
         ...
         if (strstr(deviceModel,"Genie") != 0 && strstr(deviceModel,"TS") == 0)
         {
            ...
            m_SaveAndUpload.SetWindowText(_T("Save"));
            m_isGenie = TRUE;
            UpdateData(FALSE);
            return FALSE;
         }
      ...
      for (int i = 0; i < fileCount; i++)
      {
         ...
         {
            CString featureName;
            featureName.Format(_T("%s%s"), "##FileSelector.",currentFileName);
            fileName.Format(_T("%s"),currentFileName);
            if (fileName.Find(_T("0"),0) == -1)
            {
               if(m_pFlatField->GetAcqDevice()->GetFeatureInfo(CStringA(featureName),&feature))
               {
                  char displayName[512];
                  feature.GetDisplayName(displayName,sizeof(displayName));
                  m_FlatFieldSelector.AddString(CString(displayName));               
               }
               else
               {
                  m_FlatFieldSelector.AddString(CString(currentFileName)); 
               }
...
void CFlatFieldDlg::OnDarkImage() 
{
   ...
   else
   {
      ...
      {
         LogMessage(Warning, _T("Image file has a different format than expected.  Pixel values may get shifted."));
      }

      if( loadBuffer.GetWidth() != m_pBuffer->GetWidth() || loadBuffer.GetHeight() != m_pBuffer->GetHeight() )
      {
         LogMessage(Error, _T("Image file selected doesn't have same dimensions as buffer."));
         goto Cleanup;
      }
      ...
      m_pLocalBuffer->Copy(&loadBuffer);
		...
		CString str;
      str.Format( _T("Loaded dark image: '%s'"), (LPCTSTR)dlg.GetPathName());
		LogMessage( Info, str);
   }
...
void CFlatFieldDlg::DarkImage()
{
   CString str;
	...
	str.Format( _T("Correction type: '%s'"), m_CorrectionType == SapFlatField::CorrectionTypeField? _T("Field"): _T("Line"));
   LogMessage(Info, str);

	str.Format( _T("Video type: '%s'"), m_VideoType == SapAcquisition::VideoMono ? _T("Monochrome"): _T("Bayer"));
   LogMessage(Info, str);

	if( m_pXfer != NULL)
	{
		str.Format( _T("Number of frames to average: %d"), m_NumFramesAverage);
	   LogMessage(Info, str);
	}

	if( m_CorrectionType == SapFlatField::CorrectionTypeLine)
	{
		str.Format( _T("Number of lines to average: %d"), m_NumLinesAverage);
	   LogMessage(Info, str);

		str.Format( _T("Vertical offset from top: %d"), m_VerticalOffset);
	   LogMessage(Info, str);	
	}

   LogMessage(Info, _T("Dark image calibration"));

   if (!m_pFlatField->GetStats(m_pLocalBuffer, &stats))
   {
      LogMessage(Error, _T("   Unable to get image statistics"));
      return;
   }
...
//#define USE_STANDARD_DEVIATION
   ...
   {
      str.Format(_T("The following statistics have been computed on the dark image: \n")
                 _T("    The average pixel value is %s\n")
#ifdef USE_STANDARD_DEVIATION
                 _T("    The standard deviation is %s\n")
#endif
                 _T("\nThis yields too many bad pixels above the hardware limit of %lu\n")
                 _T("\nTo disable bad pixels, uncheck the \'Consider as defective ...\'\n")
                 _T("checkbox, then acquire the dark image again"),
   ...
   str.Format(_T("    Average pixel value: %s"), (LPCTSTR)GetAverageStr( stats));
	LogMessage(Info, str);

#ifdef USE_STANDARD_DEVIATION
   str.Format(_T("    Standard deviation: %s"), (LPCTSTR)GetStdDeviationStr( stats)));
	LogMessage(Info, str);
#endif

	str.Format(_T("    Maximum deviation allowed from average pixel value: %lu"), m_DeviationMaxBlack);
   LogMessage(Info, str);
   ...
   {		
      ...
      GetDlgItem(IDC_SCG_FF_BRIGHT_IMAGE)->EnableWindow(TRUE);
	   ...
	   m_CorrectionTypeCtrl.EnableWindow( FALSE);
		...
		LogMessage(Info, _T("Calibration with a dark image has been done successfully"));
...
void CFlatFieldDlg::OnBrightImage() 
{
	...
	if (m_pXfer && *m_pXfer)
   {
      ...
      if (!Snap())
      {
         LogMessage(Error, _T("Unable to acquire an image"));
         goto Cleanup;
      }
   }
   else
   {
      ...
      m_pLocalBuffer->Copy(&loadBuffer);
		...
		CString str;
      str.Format( _T("Loaded bright image: '%s'"), (LPCTSTR)dlg.GetPathName());
		LogMessage( Info, str);
	}
...
void CFlatFieldDlg::BrightImage()
{
   ...
   SapFlatFieldStats stats;
	...
	LogMessage(Info, _T("Bright image calibration"));

   // Get statistics on the (bright - dark) image
   if (!m_pFlatField->GetStats(m_pLocalBuffer, &stats))
   {
      LogMessage(Error, _T("Unable to get image statistic"));
      return;
   }

   str.Format(_T("The following statistics have been computed on the bright image\n")
              _T("after the substraction of the dark image:\n\n")
              _T("    The average pixel value is %s.\n")
				  _T("    The highest peak has been detected at %s.\n")
              _T("    %s pixels (%s) have a luminance value between %s to %s\n")
              _T("\nWe recommend at least %d for the highest peak value\n")
              _T("with %.2f%% pixels lying between the lower and the higher bound.\n")
              _T("\nDo you want to use this image?"),
              (LPCTSTR)GetAverageStr( stats), 
   ...
   str.Format(_T("    Average pixel value: %s"), (LPCTSTR)GetAverageStr( stats));
   LogMessage(Info, str);
	str.Format(_T("    Maximum deviation allowed from average pixel value: %lu"), m_DeviationMaxWhite);
   LogMessage(Info, str);
   str.Format(_T("    Highest peak position: %s"), (LPCTSTR)GetPeakPositionStr( stats));
   LogMessage(Info, str);
   str.Format(_T("    Lower bound: %s"), (LPCTSTR)GetLowStr( stats));
   LogMessage(Info, str);
   str.Format(_T("    Upper bound: %s"), (LPCTSTR)GetHighStr( stats));
   LogMessage(Info, str);
   str.Format(_T("    Number of pixels inside bounds: %s (%s)"), (LPCTSTR)GetNumPixelsStr( stats), (LPCTSTR)GetPixelRatioStr( stats));
   LogMessage(Info, str);
   ...
   if (m_pFlatField->ComputeGain(m_pLocalBuffer, &defects, TRUE))
   {
      ...
      {
         str.Format(_T("%lu pixels (%.2f%% ) have been identified as being defective\n")
                     _T("with %lu clusters.\n")
      ...
      str.Format(_T("    Number of defective pixels detected: %lu (%.2f%%)"), defects.GetNumDefects(), defects.GetDefectRatio());
      LogMessage(Info, str);

      // Log number of cluster detected
      str.Format(_T("    Number of clusters detected: %lu"), defects.GetNumClusters());
      LogMessage(Info, str);
      ...
      m_CalibrationIndexCtrl.EnableWindow( m_pFlatField->GetNumFlatField() > 1);
		...
		LogMessage(Info, _T("Calibration with a bright image has been done successfully"));
...
CString CFlatFieldDlg::GetAverageStr( SapFlatFieldStats& stats) const
{
	...
	if( stats.GetNumComponents() > 1)
	{
		...
		{
			CString szComponent;;
			szComponent.Format( _T("%d"), stats.GetAverage( iComponent));
			str+= szComponent;
	...
	{
		str.Format( _T("%d"), stats.GetAverage());
	}
...
CString CFlatFieldDlg::GetStdDeviationStr( SapFlatFieldStats& stats) const
{
	...
	if( stats.GetNumComponents() > 1)
	{
		...
		{
			CString szComponent;;
			szComponent.Format( _T("%d"), stats.GetStdDeviation( iComponent));
			str+= szComponent;
	...
	{
		str.Format( _T("%d"), stats.GetStdDeviation());
	}
...
CString CFlatFieldDlg::GetLowStr( SapFlatFieldStats& stats) const
{
	...
	if( stats.GetNumComponents() > 1)
	{
		...
		{
			CString szComponent;;
			szComponent.Format( _T("%d"), stats.GetLow( iComponent));
			str+= szComponent;
...
CString CFlatFieldDlg::GetHighStr( SapFlatFieldStats& stats) const
{
	...
	if( stats.GetNumComponents() > 1)
	{
		...
		{
			CString szComponent;;
			szComponent.Format( _T("%d"), stats.GetHigh( iComponent));
			str+= szComponent;
...
CString CFlatFieldDlg::GetPeakPositionStr( SapFlatFieldStats& stats) const
{
	...
	if( stats.GetNumComponents() > 1)
	{
		...
		{
			CString szComponent;;
			szComponent.Format( _T("%d"), stats.GetPeakPosition( iComponent));
			str+= szComponent;
	...
	{
		str.Format( _T("%d"), stats.GetPeakPosition());
	}
...
CString CFlatFieldDlg::GetPixelRatioStr( SapFlatFieldStats& stats) const
{
	...
	if( stats.GetNumComponents() > 1)
	{
		...
		{
			CString szComponent;;
			szComponent.Format( _T("%.2f%%"), stats.GetPixelRatio( iComponent));
			str+= szComponent;
	...
	{
		str.Format( _T("%.2f%%"), stats.GetPixelRatio());
	}
...
CString CFlatFieldDlg::GetNumPixelsStr( SapFlatFieldStats& stats) const
{
	...
	if( stats.GetNumComponents() > 1)
	{
		...
		{
			CString szComponent;;
			szComponent.Format( _T("%d"), stats.GetNumPixels( iComponent));
			str+= szComponent;
	...
	{
		str.Format( _T("%d"), stats.GetNumPixels());
	}
...
{
   CFileDialog dlg( FALSE, _T(""), _T("FFC.tif"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("TIFF Files (*.tif)|*.tif|"), this);
	
	if( dlg.DoModal() == IDOK)
	{
		// Save flat field offset correction file
		m_pFlatField->Save(CStringA(dlg.GetPathName()));
      ...
      if (m_UserFlatFieldCount > 1 && !m_isGenie)
      {
         ...
         if (m_pFlatField->GetAcqDevice()->WriteFile(CStringA(dlg.GetPathName()),index))
