...
BOOL CLoadSaveDlg::AddTiffOptions()
{
   ...
   if (dlg.DoModal() != IDOK)
	...
	CString strType;
   strType.Format(CString(" -compression %s"), (LPCTSTR)m_TiffCompressionTypes[dlg.GetCompressionType()]);
	m_Options += strType;

	CString strLevel;
	strLevel.Format(CString(" -quality %d"), dlg.GetCompressionLevel());
	m_Options += strLevel;
...
BOOL CLoadSaveDlg::AddRawOptions()
{
   ...
   m_RawFileFormat = dlg.GetRawFileFormat();
	...
	CString str;
	str.Format(CString(" -width %d -height %d -offset %d"), m_RawFileWidth, m_RawFileHeight, m_RawFileOffset);
	m_Options += str;
...
BOOL CLoadSaveDlg::AddJpegOptions()
{
   ...
   if (dlg.DoModal() != IDOK)
	...
	CString str;
	str.Format(CString(" -q %d"), dlg.GetJpegFileQuality());
	m_Options += str;
