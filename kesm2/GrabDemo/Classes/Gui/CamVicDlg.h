...
protected:
	...
	void SetDirectories();
	void UpdateCurrentDir(TCHAR *newCurrentDir, BOOL bCamFile);
...
protected:
   ...
   CString m_ProductDir;      // Initial directory for finding CAM and VIC files
	...
	TCHAR m_CamFile[MAX_PATH];						// Complete CAM path
	TCHAR m_VicFile[MAX_PATH];						// Complete VIC path

	TCHAR m_currentCamDir[MAX_PATH];			// Directory where to find the CCA files
	TCHAR m_currentCamFileName[MAX_PATH];	// File name (without path) of the current file selected
	int m_currentCamFileIndex;					// Index of the file name in the list current selected

	TCHAR m_currentVicDir[MAX_PATH];			// Directory where to find the CVI files
	TCHAR m_currentVicFileName[MAX_PATH];	// File name (without path) of the current file selected
	...
	TCHAR m_makeVicFile[MAX_PATH];				// MakeVic utility path
};
