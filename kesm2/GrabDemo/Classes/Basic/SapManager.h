...
public:
	...
	SapLocation() 
      { m_ServerIndex = ServerSystem; CorStrncpy(m_ServerName, "", CORSERVER_MAX_STRLEN); m_ResourceIndex = 0; }

	SapLocation(int serverIndex, int resourceIndex = 0) 
      { m_ServerIndex = serverIndex; CorStrncpy(m_ServerName, "", CORSERVER_MAX_STRLEN); m_ResourceIndex = resourceIndex; }

	SapLocation(const char *serverName, int resourceIndex = 0) 
      { m_ServerIndex = ServerUnknown; CorStrncpy(m_ServerName, serverName, CORSERVER_MAX_STRLEN); m_ServerName[CORSERVER_MAX_STRLEN - 1] = '\0'; m_ResourceIndex = resourceIndex; }

	SapLocation(int serverIndex, const char *serverName, int resourceIndex) 
      { m_ServerIndex = serverIndex; CorStrncpy(m_ServerName, serverName, CORSERVER_MAX_STRLEN); m_ServerName[CORSERVER_MAX_STRLEN - 1] = '\0'; m_ResourceIndex = resourceIndex; }

	SapLocation(const SapLocation &loc) 
      { m_ServerIndex = loc.m_ServerIndex; CorStrncpy(m_ServerName, loc.m_ServerName, CORSERVER_MAX_STRLEN); m_ResourceIndex = loc.m_ResourceIndex; }

	SapLocation(const SapLocation &loc, int resourceIndex) 
      { m_ServerIndex = loc.m_ServerIndex; CorStrncpy(m_ServerName, loc.m_ServerName, CORSERVER_MAX_STRLEN); m_ResourceIndex = resourceIndex; }
...
protected:
   void Construct(SapManager::EventType eventType, int serverIndex, int resourceIndex, void *context, SAPSTATUS errorValue, const char *errorMessage, int percentProgress)
   {
      ...
      m_FilePercentProgress = percentProgress;
      if (errorMessage)
      {
         // Make sure that the resulting string is null terminated, no matter how CorStrncpy is defined
         CorStrncpy(m_ErrorMessage, errorMessage, MaxMessageSize);
