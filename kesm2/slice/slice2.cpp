...
//cutting parameters
double YSliceSize		=	1.0;
double ZSliceSize		=	0.005;
double CuttingSpeed		=	5;
double CuttingSpeedVariance = 0.2;
double SlicesPerBlock	=	10;
int StairBlockDepth		=	5;
int StairBlockWidth		=	5;

double LiftHeight		=	0.1;
double ReturnSpeed		=	21;
double XSliceSize		=	16;
...
void ChangeCuttingOptionsMenu(char menu)
{
	...
	case '2':
		...
		break;
	case '3':
		cout<<"Enter new Cutting Speed ("<<CuttingSpeed<<"mm/s): ";
		cin>>CuttingSpeed;
		cout<<endl;
		break;
	case '4':
...
void CuttingOptionsMenu(char menu)
{
	...
	case '1':
		while(menuoption != '0')
		{
			...
			cout<<"(3) Cutting Speed    = "<<CuttingSpeed<<endl;
	...
	case '3':		//display cutting parameters
		...
		cout<<"Cutting Speed:   "<<CuttingSpeed<<"mm/s"<<endl;
...
bool CutSlice()
{
	double speed_variance = CuttingSpeed*CuttingSpeedVariance;
	double speed_min = CuttingSpeed - speed_variance;
	double speed_max = CuttingSpeed + speed_variance;
	double random_variance = speed_variance*2.0 * ((double)rand()/(double)RAND_MAX);
	double speed = speed_min + random_variance;
	...
	WaitABit(1);	//wait a second

	//turn camera firing on
	SendCommand("fire ON");


	if(!IncrementStage(XSliceSize, 0,0,speed))
...
void SliceSession()
{
	...
	while(keypress != '0')
	{
		...
		else if(keypress == '+')
		{
			...
			if(ZSliceSize > 0.1)
			{
				cout<<"Error, upper thickness of knife is 100 microns"<<endl;
				ZSliceSize=0.1;
			}
			else
		...
		else if(keypress == '-')
		{
			...
			{
				cout<<"Error, Z step can't go below zero, genius"<<endl;
				ZSliceSize=DYNAMIC_SLICE_STEP;
			}
			else
		...
		{
			CutSlice();
			cout<<"Cutting Slice at "<<speed<<"mm/sec"<<endl;
			TotalZ+=ZSliceSize;
			TotalSlices++;
		}
...
void InitializeStage()
{
	...
	{
		error = WAPIAerSoftwareInitialize ( "c:\\u500\\mmi\\U500.prm" ) ; // resets system software without hardware reset
		//error = WAPIAerInitialize ( NULL, "c:\\u500\\mmi\\u500c.jwp", "c:\\u500\\mmi\\bts.prm");
	}
