/*
 *
 *	pxlibvs.h	External	21-Jan-2006
 *
 ...
 */
     ...
     * It remains so, even though 64 bit buffer indices are unlikely;
     * but the size of some video state structures will change.
     */
    typedef long	    pxbuffer_t;     /* an image buffer number	   */
    typedef uint32	    pxvbtime_t;     /* a line/field/frame count    */
    typedef uint64	    pxapiadrs_t;    /* Added (temporary?) to facilitate */
    typedef uint64	    pximaplen_t;    /* conversion to 64 bit o.s.	*/
#else
    typedef long	    pxbuffer_t;     /* an image buffer number	   */
...
struct	pxvidformat {
	...
	int	pixdopes[PXMAX_LOVC];	/* internal			    */
			    ...
			    /* PIXCI D/D24/D32/D2X/CL1/CL2/E1/E4/E1DB/SI:   */
...
struct	pxviddim {	    /* common control over x and y			    */
    ...
    int     vidsamples;     /* video pixels per line/column			    */
			    ...
			    /* XC D/D24/D32/A/D2X/CL1/CL2/E1/E4/E1DB/SI/CL3SD only  */
			    /* allow  equality					    */
    ...
    int     setmaxdatsamples;  /* 0|1: force data samples per line/field to max     */
    int     setmaxvidsamples;  /* 0|1: force video samples per line/field to max    */
...
struct	pxvidres {
	...
	int	setpixbits;	/* 0|1|2: force pixies/pixhint/pixiebits to max 	*/
			    ...
			    /* CL2/E1/E4/E1DB uses both dat.* and vid.* 		*/
