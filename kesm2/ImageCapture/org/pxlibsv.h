/*
 *
 *	pxlibsv.h	External	05-Jul-2004
 *
 ...
 * Types.
 *
 * The _farimap is intentionally an adjective but not
...
#define PXMAX_GPIN	8	/* max general purpose inputs			*/
#define PXMAX_IRBGF	8	/* worst case # of pximage structs required	*/
				/* by initPximage generator functions - not	*/
				/* the maximum number of images supported!	*/
				/* Future: distinguish max for 2-d vs 3-d?	*/
...
struct pxvidstatus {
     ...
     * is never 0, is quite appripriate as the status may have actually
...
#define PXMOS_VIDSTATUS (PXMOS_DDCH+PXMOS_TIMESTAMP+3+5+8+2*(8+1)+2+PXMAX_GPIN*2)
...
struct pxdevfault {
    ...
    int 	unitmap;	/* bitmap of units on which fault occurred  */
				/* may be 0 if fault not associated with    */
...
#define PXFLT_FAILURE	0x02	/* operation failed, but in others might succeed*/
...
struct pximagebuf {
    ...
    void   *buf[8];	/* internal use */
    uint    len[8];	/* internal use */
    pxapiadrs_t fad[8]; /* internal use */
  //void    (*destruct)(pximagebuf_s *me);
};
struct pximage3buf {
    struct  pxddch  ddch;
    ...
    uint    len[8];	/* internal use */
    pxapiadrs_t fad[8]; /* internal use */
  //void    (*destruct)(pximage3buf_s *me);
};
typedef struct pximagebuf  pximagebuf_s;
typedef struct pximage3buf pximage3buf_s;
#define PXMOS_PXIMAGEBUF    (PXMOS_DDCH+PXMAX_IRBGF+8+8+8)
...
struct pxbufstatusfile {
    ...
    uint32	captticks[2];
    uint32	captgpin;
    char	rsvd[64-sizeof(uint16)*2-sizeof(uint32)*4];

};
#define PXMOS_BUFSTATUSFILE (2+4)
...
struct pxdevservice {
    ...
    uint32 (_cfunfcc *getVidStatusValue)(pxdevservice_s *me,int unitmap,int rsvd1,int mode,uint offset);
    ...
    int (_cfunfcc *io)		 (pxdevservice_s *me,int options,int unitmap,pxapiadrs_t adrs,uint altspace,void *buf,uint cnt,uint rw,pxasync_s *async,pxtimespec_s *time);
  //int (_cfunfcc *iosparse)	 (pxdevservice_s *me,int options,int unitmap,pxapiadrs_t adrs,uint altspace,void *buf,uint cnt,uint rw,pxasync_s *async,pxtimespec_s *time,uint blksize,uint blkpitch);

    /*
     * Misc control of misc auxilliary features.
     */
    int (_cfunfcc *ioctl)	 (pxdevservice_s *me,int options,int unitmap,uint32 adrs,uint space,uint subspace,void *buf,uint cnt,uint rw,pxasync_s *async,pxtimespec_s *time);
...
#define PXMOS_DEVSERVICE    (1+5+3+1+1+2+PXMOS_DEVFAULT)
...
struct pxlibservice {
     ...
     */
    int (_cfunfcc *makePximageBuf)  (pxlibservice_s *me,int unitmap,pximagebuf_s **ipbuf,int colorhint,int altspace,int stateid,int mbpcihint);
    int (_cfunfcc *freePximageBuf)  (pxlibservice_s *me,int unitmap,pximagebuf_s **ipbuf,int colorhint,int altspace,int stateid,int mbpcihint);
    int (_cfunfcc *loadPximageBuf)  (pxlibservice_s *me,int unitmap,pximagebuf_s *ipbuf,int colorhint,int altspace,int stateid,pxbuffer_t buffer,int mbpcihint);
    int (_cfunfcc *savePximageBuf)  (pxlibservice_s *me,int unitmap,pximagebuf_s *ipbuf,int colorhint,int altspace,int stateid,pxbuffer_t buffer,int mbpcihint);
...
#define PXMOS_LIBSERVICE    (1+4+4+3+7+3+4+6+6)
...
struct pxauxservice {
     ...
     * Future: Rename and/or clarify these to operate on 8 bit,
     ...
     */
    int (_cfunfcc *bufferSaveStream) (pxauxservice_s *me,pxabortfunc_t**,int options,int unitmap,int stateid,pxbuffer_t startbuf,pxbuffer_t endbuf,char *pathname,void *filehandle,pxbuffer_t fileoffset,uint32 alignsector);
    int (_cfunfcc *bufferLoadStream) (pxauxservice_s *me,pxabortfunc_t**,int options,int unitmap,int stateid,pxbuffer_t startbuf,pxbuffer_t endbuf,char *pathname,void *filehandle,pxbuffer_t fileoffset,uint32 alignsector);
...
#define PXMOS_AUXSERVICE    (1+6+6+2+(2+1+1+3)+1+1)
