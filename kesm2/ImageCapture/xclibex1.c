/*
 *
 *	xclibex1.c	External	15-Mar-2006
 *
 *	Copyright (C)  1996-2006  EPIX, Inc.  All rights reserved.
 *
 ...
 *	and video format.
 *
 *	For PIXCI(R) SV2, SV3, SV4, SV5, and SV5A imaging boards
 ...
 */
				...
				// For PIXCI(R) SV2, SV3, SV4, SV5, SV5A
...
void do_savetif()
{
	...
	if ((i = pxd_saveTiff(1<<u, name, 1, 0, 0, -1, -1, 0, 0)) < 0) {
...
void do_savebmp()
{
	...
	if (UNITS == 1)
	    ...
	    printf("Unit %d Image buffer 1 being saved to file %s\n", u, name);
	if ((i = pxd_saveBmp(1<<u, name, 1, 0, 0, -1, -1, 0, 0)) < 0) {
...
void do_savejpeg()
{
	...
	for (u = 0; u < UNITS; u++) {
		...
		printf("Unit %d Image buffer 1 being saved to file %s\n", u, name);
	    if ((i = pxio8_jpegwrite(NULL, pxd_defineImage(u,1,0,0,-1,-1,"RGB"), NULL, name, 8, NULL)) < 0) {
...
void do_vgadisplay()
{
	...
	pxio8_vgadisplay(NULL, pxd_defineImage(1<<u, 1, 0, 0, -1, -1, "Display"), NULL,
...
void hello(void)
{
    ...
    printf("Copyright (C)  1996-2006  EPIX, Inc.  All rights reserved.\n");
    printf("\n");
