/*
 *
 *	xclibex2.cpp	External	11-Jul-2005
 *
...
// about 50,000 pixel height
// 50,000/4096 = 12.xxx
#define sMAX_FRAME_BUFFER		1  // pxd_imageZdim() //13	
 ...
 *
 */
#define LIVE_LIVE	0//1   // Continuous capture into frame buffer,
			    // display from same frame buffer while
			    // next frame is being captured.
#define LIVE_SNAP	1//0   // Snap single frame into frame buffer,
			    // display, repeat.
...
char g_szFileName[_MAX_PATH];  // from StageController app
char g_szVidSetName[_MAX_PATH];  // loaded from file during execution
...
void InitDialog(HWND hDlg, UINT svgaBits, struct pxywindow windImage[4])
{
	HWND hWndImage;
	...
	sprintf(driverparms, "-DM 0x%x %s -MU 0", UNITSMAP, DRIVERPARMS);
	//
	...
	//
	    ...
	    if (pxd_PIXCIopen(driverparms, "", g_szVidSetName) < 0)
		pxd_mesgFault(UNITSMAP);
...
//	SetWindowText(hDlg, "EPIX(R) PIXCI(R) Imaging Board Example");
	...
	//
		...
		//Event = pxd_eventGPTriggerCreate(0x1, 1, 0);
	    if (!Event)
...
BOOL FileOrDirExists(char *szFileOrDirname)
{
   WIN32_FIND_DATA entry;

   HANDLE hFile;
   hFile = FindFirstFile(szFileOrDirname, &entry);
   FindClose(hFile);
...
PIXCIDialogProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (wMsg) {
    case WM_INITDIALOG:
		InitDialog(hDlg, svgaBits, windImage);
		hWndImage = GetDlgItem(hDlg, IDIMAGE);
	...
	case IDC_BUTTON_LOAD:
			...
			return(FALSE);
	    {
			...
			ofn.hwndOwner	= hWnd;
			ofn.lpstrFilter = "VidSet Files (*.fmt)\0*.fmt\0\0";
			ofn.lpstrFile	= pathname;
			ofn.nMaxFile	= _MAX_PATH;
			ofn.lpstrTitle	= "Video Settings";
			ofn.Flags   |= OFN_EXPLORER|OFN_OVERWRITEPROMPT|OFN_HIDEREADONLY;
			r = GetOpenFileName(&ofn);
			if (r != 0) {
				char *pFileName = strrchr(pathname, '\\');
				if(pFileName)
					pFileName++;
				else
					pFileName = pathname;

				if(strcmp(g_szVidSetName, pFileName))
				{
					strcpy(g_szVidSetName, pFileName);
					pxd_PIXCIclose();
					InitDialog(hDlg, svgaBits, windImage);

					FILE *fp;
					fp = fopen("VidSet.txt", "wt");
					if(fp)
					{
						fputs(g_szVidSetName, fp);
						fclose(fp);
					}
				}
			}
	...
	case IDSNAP:
		...
		pxd_doSnap(UNITSMAP, 1, 0);
		{
			...
			for(int i = 0; liveon && i < sMAX_FRAME_BUFFER; i++)
			{
				...
				start = clock();
				r = pxd_saveTiff(0x1, szSaveName, i+1, 0, 0, -1, -1, 0, 0);
				//r = pxd_saveRawBuffers(0x1, szSaveName, 1, sMAX_FRAME_BUFFER, fp, 0, 0, 0);
				...
				char szMsg[256];
				sprintf(szMsg, "(%d) Elapsed seconds: %2.1f\n", i, duration);
		...
		// capturing is done
		{
			...
			if(!bError && hWnd)
			{
				PostMessage(hWnd, WM_APP_BSS, WP_DC_SAVE_DONE, 0);
			}
		}
		liveon = false;
	...
	case WM_APP_BSS:
	{
		...
		if(wParam == WP_START_CAPTURE)
		{
			...
			PostMessage(hDlg, WM_COMMAND, MAKEWORD(IDSNAP, BN_CLICKED), 0);
			return TRUE;
		}
	...
	if (pxd_infoUnits()) {	 // implies whether librry is open
		...
		char  buf[100];
		sprintf(buf, "dim: %dx%d  res: %dx%d  fps: %.5g disp: %.5g",
		...
		#if defined(FORMAT)
		    sprintf(buf, "fmt: %s", FORMAT);
		    SetWindowText(GetDlgItem(hDlg, IDSTATUS2), buf);
		#elif defined(FORMATFILE_LOAD)
		    //sprintf(buf, "fmt: %s", FORMATFILE_LOAD);
		    sprintf(buf, "fmt: %s", g_szVidSetName);
		    SetWindowText(GetDlgItem(hDlg, IDSTATUS2), buf);
		#elif defined(FORMATFILE_COMP)
		    sprintf(buf, "fmt: %s", "compiled");
		    SetWindowText(GetDlgItem(hDlg, IDSTATUS2), buf);
		#endif
		lasttickcount  = ticks;
    ...
    case WM_USER:	    // if using UPDATE_EVENT, update image display
		...
		/* don't display at all for speeding */
		//break;
//#if 0 
	//
...
int APIENTRY WinMain(
	...
	FILE *fp;
	fp = fopen("VidSet.txt", "rt");
	if(fp)
	{
		fgets(g_szVidSetName, 256, fp);
		fclose(fp);
	}
	else
		strcpy(g_szVidSetName, "xcvidset_4096x24968.fmt");
