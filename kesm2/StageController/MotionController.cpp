...
bool CMotionController::CutSlice(double dCuttingSpeed)
{
	...
	// jwyoo, hwang, 02-26-2016, for debugging
	/*
	if(pWnd == NULL)
	{
		CommandLog("Can not find the Data Capture Window");
		//return false;
	} else {
		CommandLog("Can find the Data Capture Window");
	}
	*/

	// jwyoo, hwang, 02-29-2016
	bool ispDCaptureWnd = false;
    ispDCaptureWnd = ((CStageControllerDlg *)m_pParentWnd)->GetnSetpDCapture();
	
	//if(pWnd && ((CStageControllerDlg*)m_pParentWnd)->m_fSaveImages)
	if(ispDCaptureWnd && ((CStageControllerDlg*)m_pParentWnd)->m_fSaveImages)
	{
		CTime CurTime = CTime::GetCurrentTime();
		...
		//jwyoo, hwang, 02-29-2016
		((CStageControllerDlg *)m_pParentWnd)->SendMessage2DCapture(WM_COPYDATA, (WPARAM)AfxGetApp()->m_pMainWnd->GetSafeHwnd(), (LPARAM)&cpd);
		((CStageControllerDlg *)m_pParentWnd)->PostMessage2DCapture(WM_APP_BSS, WP_START_CAPTURE); 
